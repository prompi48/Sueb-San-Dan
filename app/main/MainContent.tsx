...
แยก Component หลักของหน้า Main ออกมาเพื่อความเป็นระเบียบและง่ายต่อการจัดการ

ในไฟล์นี้จะมี Logic หลักทั้งหมดของหน้า Main 
ตั้งแต่การ Fetch Data, การจัดการ User Session, การทำ Pagination, การทำ Search, และการจัดการ User Settings

.range(from, to): ใช้สำหรับ Pagination โดยกำหนดช่วงของข้อมูลที่ต้องการดึงจากฐานข้อมูล เช่น ถ้าอยู่หน้า 2 และแสดงผล 30 รายการต่อหน้า ก็จะดึงข้อมูลตั้งแต่รายการที่ 30 ถึง 59
.eq('is_pending', false): ใช้สำหรับกรองข้อมูลเฉพาะโพสต์ที่ผ่านการอนุมัติแล้วเท่านั้น (is_pending = false)

Search Logic: ถ้ามีการกรอกข้อมูลในช่อง Search จะมีการปรับ Query ที่ใช้ดึงข้อมูลจากฐานข้อมูลให้ตรงกับเงื่อนไขที่กรอกเข้ามา เช่น 
ถ้ากรอกชื่อเรื่อง ระบบจะใช้ ilike() เพื่อค้นหาชื่อเรื่องที่มีคำที่กรอกอยู่ในนั้น (ไม่ต้องตรงทั้งหมดและไม่สนใจตัวพิมพ์ใหญ่เล็ก)
ถ้ากรอกชื่อผู้เขียน ระบบจะไปค้นหาก่อนในตาราง profiles เพื่อหาว่ามีใครที่ชื่อใกล้เคียงกับที่กรอกเข้ามาบ้าง แล้วเอา id ของคนเหล่านั้นไปใช้ค้นหาโพสต์ที่มี author_id ตรงกับ id เหล่านั้น
และต้องกด execute search เพื่อให้ระบบทำการค้นหาและอัปเดตผลลัพธ์ในหน้า Main
*/
'use client';
